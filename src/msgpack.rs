...
impl<'a> Decoder<'a> {
  ...
  fn decode_value(&mut self) -> Value {
    ...
    match c {
      ...
      0xa0 .. 0xbf => String(self._read_str((c as uint) & 0x1F)),
      0xd9         => { let b = self.rd.read_u8().unwrap() as uint; String(self._read_str(b)) },
      0xda         => { let b = self.rd.read_be_u16().unwrap() as uint; String(self._read_str(b)) },
      0xdb         => { let b = self.rd.read_be_u32().unwrap() as uint; String(self._read_str(b)) },
...
impl<'a> serialize::Encodable<Encoder<'a>> for Value {
  fn encode(&self, s: &mut Encoder<'a>) {
    match *self {
      ...
      String(ref str) => (s as &mut serialize::Encoder).emit_str(str.slice_from(0)),
      Binary(_) => fail!(),
      Extended(_, _) => fail!()
    }
  }
