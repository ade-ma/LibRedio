...
extern "C" {
	...
	fn pa_simple_read(s: *PASimple, data: *mut c_void, bytes: size_t, error: *c_int) -> c_int;
	fn pa_simple_write(s: *PASimple, data: *c_void, bytes: size_t, error: *c_int) -> c_int;
	fn pa_simple_flush(s: *PASimple, error: *c_int) -> c_int;
	fn pa_simple_get_latency(s: *PASimple, error: *c_int) -> u64;
}
...
pub fn pulseSource(cData: comm::Sender<Vec<f32>>, sRate: uint, bSize: uint) {
	...
	unsafe {
		let error: c_int = 0;
		let s: *PASimple = pa_simple_new(null(), "rust-pa-simple-source".to_c_str().unwrap(), 2, null(), "pa-source".to_c_str().unwrap(), &ss, null(), null(), &error);
		assert_eq!(error, 0);
		'main : loop {
...
pub fn pulseSink(pData: Receiver<Vec<f32>>, sRate: uint) {
	...
	let error: c_int = 0;
	unsafe {
		let s: *PASimple = pa_simple_new(null(), "rust-pa-simple-sink".to_c_str().unwrap(), 1, null(), "pa-sink".to_c_str().unwrap(), &ss, null(), null(), &error);
