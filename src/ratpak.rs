...
pub fn main() {
    ...
    task::TaskBuilder::new().named("rtl_source_cmplx").spawn(|| {
                                                             rtl_source_cmplx(txN078001N079001,
                                                                              433900000, 402, 256000)
                                                         });
    task::TaskBuilder::new().named("cross_applicator_vecs").spawn(|| {
                                                                  cross_applicator_vecs(rxN078001N079001,
                                                                                        txN079001N080001,
                                                                                        |x|{x.norm()})
                                                              });
    task::TaskBuilder::new().named("trigger").spawn(|| {
                                                    trigger(rxN079001N080001,
                                                            txN080001N081001)
                                                });
    task::TaskBuilder::new().named("discretize").spawn(|| {
                                                       discretize(rxN080001N081001,
                                                                  txN081001N082001)
                                                   });
    task::TaskBuilder::new().named("rle").spawn(|| {
                                                rle(rxN081001N082001,
                                                    txN082001N083001) });
    task::TaskBuilder::new().named("fork").spawn(|| {
                                                 fork(rxN083001N0840,
                                                      [txN083001N084001,
                                                       txN083001N084002]) });
    task::TaskBuilder::new().named("dle").spawn(|| {
                                                dle(rxN082001N083001,
                                                    txN083001N0840, 256000)
                                            });
    task::TaskBuilder::new().named("looper").spawn(|| {
                                                   looper(rxN083001N084001,
                                                          txN084001N085001,
                                                          |mut a,b| {a.map(|x| {b.send(match x { (1, 2e-4...6e-4) => match a.next().unwrap() { (0, 1.5e-3...2.5e-3) => Some(0us), (0, 3.5e-3...4.5e-3) => Some(1us), _ => None}, _     => None})}).last();()})
                                               });
    task::TaskBuilder::new().named("looper").spawn(|| {
                                                   looper(rxN083001N084002,
                                                          txN084002N085002,
                                                          |mut a,b| {a.map(|x| {b.send(match x {(1, ref d@125e-6...250e-6) | (1, ref d @500e-6...650e-6) => {match a.next().unwrap() {(0, ref e @500e-6...650e-6) | (0, ref e@125e-6...250e-6) => Some(if d > e {1us} else {0us}),_ => None}}, _ => None})}).last();()})
                                               });
    task::TaskBuilder::new().named("fork").spawn(|| {
                                                 fork(rxN085001N0860,
                                                      [txN085001N086001,
                                                       txN085001N086002]) });
    task::TaskBuilder::new().named("shaper_optional").spawn(|| {
    ...
    task::TaskBuilder::new().named("binconv").spawn(|| {
    ...
    task::TaskBuilder::new().named("binconv").spawn(|| {
                                                    binconv(rxN085001N086002,
                                                            txN086002N087002,
                                                            [4,8,2,10,12]) });
    task::TaskBuilder::new().named("applicator").spawn(|| {
                                                       applicator(rxN085002N086003,
                                                                  txN086003N090003,
                                                                  |mut x|{x.push(0); x})
                                                   });
    task::TaskBuilder::new().named("cross_applicator").spawn(|| {
    ...
    task::TaskBuilder::new().named("looper_optional").spawn(|| {
    ...
    task::TaskBuilder::new().named("mul_vecs").spawn(|| {
    ...
    task::TaskBuilder::new().named("grapes").spawn(|| {
                                                   grapes([rxN090001N091001,
                                                           rxN090002N091001,
                                                           rxN090003N091001],
                                                          txN091001N092001)
                                               });
    task::TaskBuilder::new().named("differentiator").spawn(|| {
    ...
    task::TaskBuilder::new().named("print_sink").spawn(|| {
